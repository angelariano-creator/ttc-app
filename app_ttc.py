                ...
                if seleccion:
                    ...
                    if cuota_inicial_real > 0:
                        ...
                        if seccion == "📋 Contacto":
                            ...
                            saldo_ci = cuota_inicial_automatica - total_cancelado_ci_real
                            ...
                            with c1:
                                ...
                                <div class="card-financiera">
                                    <div class="card-titulo">🏠 Valor Venta</div>
                                    <div class="card-valor">{formato_moneda(venta)}</div>
                                </div>
                                """, unsafe_allow_html=True)
                            ...
                            with c2:
                                st.markdown(f"""
                                <div class="card-financiera">
                                    <div class="card-titulo">💰 Cuota Inicial</div>
                                    <div class="card-valor">{formato_moneda(cuota_inicial_automatica)}</div>
                                </div>
                                """, unsafe_allow_html=True)
                            ...
                            with c3:
                                st.markdown(f"""
                                <div class="card-financiera">
                                    <div class="card-titulo">💵 Abonado</div>
                                    <div class="card-valor">{formato_moneda(total_cancelado_ci_real)}</div>
                                </div>
                                """, unsafe_allow_html=True)
                            ...
                            with c4:
                                st.markdown(f"""
                                <div class="card-financiera">
                                    <div class="card-titulo">📉 Saldo CI</div>
                                    <div class="card-valor">{formato_moneda(saldo_ci)}</div>
                                </div>
                                """, unsafe_allow_html=True)
                            ...
                            st.markdown("### 📊 Avance de Pago")

                            progreso = min(int(porcentaje_ci),100)

                            st.progress(progreso)

                            st.caption(f"Cumplimiento de cuota inicial: **{porcentaje_ci:.2f}%**")
                            ...
                            if porcentaje_ci >= 100:
                                ...
                                ✅ Cuota inicial completamente pagada
                                </div>
                                """, unsafe_allow_html=True)

                            elif porcentaje_ci >= 70:

                                st.markdown("""
                                <div class="estado-amarillo">
                                🟡 Buen progreso en el pago de la cuota inicial
                                </div>
                                """, unsafe_allow_html=True)
                                ...
                                ⚠️ La cuota inicial aún tiene saldo pendiente
                                </div>
                                """, unsafe_allow_html=True)
                            ...
                            col5.metric(
                                "Saldo Total Apartamento",
                                formato_moneda(saldo_real)
                            )                                               
                            # ==================================================
                            # 📅 GESTIÓN UNIFICADA DE PLAN DE PAGOS (12 CUOTAS)
                            # ==================================================
                            st.markdown("---")
                            st.subheader("📅 Plan de Pagos")

                            # 1. ACORDEÓN DE CONFIGURACIÓN (Compacto y Alineado)
                            with st.expander("⚙️ Configurar / Editar Cuotas"):
                                # Selector de cuotas más pequeño (ocupando 1/3 del ancho)
                                ...
                                st.write("") # Espaciador
                                # Cabeceras alineadas con los inputs
                                h1, h2, h3 = st.columns([1.5, 1.5, 1])
                                h1.caption("**Fecha de Vencimiento**")
                                h2.caption("**Monto de la Cuota**")
                                h3.caption("**Estado**")

                                # Filas de la tabla
                                for i in range(1, cuotas_pactadas + 1):
                                    c1, c2, c3 = st.columns([1.5, 1.5, 1])
                                    
                                    with c1:
                                        val_f = cliente.get(f"FECHA_C{i}", date.today())
                                        nueva_f = st.date_input(f"f{i}", value=val_f, key=f"fecha_c{i}", label_visibility="collapsed")
                                    
                                    with c2:
                                        val_v = formato_moneda(cliente.get(f"VALOR_C{i}", 0))
                                        nuevo_v = st.text_input(f"v{i}", value=val_v, key=f"valor_c{i}", label_visibility="collapsed")
                                    
                                    with c3:
                                        est_a = cliente.get(f"ESTADO_C{i}", "PENDIENTE")
                                        nuevo_e = st.selectbox(f"e{i}", ["PENDIENTE", "PAGADO"], 
                                                            index=0 if est_a == "PENDIENTE" else 1, 
                                                            key=f"estado_c{i}", label_visibility="collapsed")

                                # Botón de guardar pequeño (ocupando 1/3 del ancho)
                                st.write("")
                                col_btn, _ = st.columns([1, 2])
                                with col_btn:
                                    if st.button("💾 Guardar Plan", type="primary", use_container_width=True):
                                        st.session_state.db.at[idx_sel, "NUMERO_CUOTAS"] = cuotas_pactadas
                                        for i in range(1, 13):
                                            if i <= cuotas_pactadas:
                                                st.session_state.db.at[idx_sel, f"FECHA_C{i}"] = st.session_state[f"fecha_c{i}"]
                                                st.session_state.db.at[idx_sel, f"VALOR_C{i}"] = limpiar_valor(st.session_state[f"valor_c{i}"])
                                                st.session_state.db.at[idx_sel, f"ESTADO_C{i}"] = st.session_state[f"estado_c{i}"]
                                            else:
                                                st.session_state.db.at[idx_sel, f"FECHA_C{i}"] = None
                                                st.session_state.db.at[idx_sel, f"VALOR_C{i}"] = 0
                                                st.session_state.db.at[idx_sel, f"ESTADO_C{i}"] = "N/A"
                                        
                                        st.session_state.db.to_excel("Base_Datos_TTC.xlsx", index=False)
                                        st.success("Plan actualizado")
                                        st.rerun()

                            # 2. LÓGICA DE ALERTA UNIFICADA (Resumen único)
                            vencidas_lista = []
                            total_vencido_acumulado = 0
                            proximas_lista = []

                            for i in range(1, cuotas_pactadas + 1):
                                f_c = cliente.get(f"FECHA_C{i}")
                                v_c = cliente.get(f"VALOR_C{i}", 0)
                                e_c = cliente.get(f"ESTADO_C{i}", "PENDIENTE")

                                if pd.notnull(f_c) and e_c == "PENDIENTE":
                                    f_obj = f_c.date() if hasattr(f_c, 'date') else pd.to_datetime(f_c).date()
                                    dif = (f_obj - date.today()).days
                                    
                                    if dif < 0:
                                        vencidas_lista.append(f"Cuota {i} (venció hace {abs(dif)} días)")
                                        total_vencido_acumulado += v_c
                                    elif 0 <= dif <= 5:
                                        proximas_lista.append(f"Cuota {i} (vence en {dif} días)")

                            # Muestra un SOLO mensaje de estado basado en la prioridad
                            if vencidas_lista:
                                msg_vencidas = "  \n".join(vencidas_lista)
                                st.error(f"🚨 **Atención: Pagos Vencidos** \n{msg_vencidas}  \n**Total en mora: {formato_moneda(total_vencido_acumulado)}**")
                            elif proximas_lista:
                                msg_proximas = "  \n".join(proximas_lista)
                                st.warning(f"⏳ **Pagos Próximos** \n{msg_proximas}")
                            elif porcentaje_ci >= 100:
                                st.success("✅ **Estado:** Cuota inicial pagada al 100%.")
                            else:
                                st.info("💡 **Estado:** El cliente no tiene cuotas vencidas ni próximas a vencer.")
                            ...
                            with st.container(border=True):
                                ...
                                with c_cred1:
                                    nuevo_ccf = st.text_input("CCF (Caja Compensación):", value=str(cliente.get(c_ccf, "")))
                                    nuevo_c_ini = st.text_input("Cuota Inicial ($):", value=formato_moneda(venta * 0.10))
                                    nuevo_can_ci = st.text_input("Total Cancelado C.I. ($):", value=formato_moneda(cancelado_ci))
                                    
                                    # --- VALIDACIÓN DE FECHA DE COMPROMISO (SOLUCIÓN AL ERROR) ---
                                    f_comp_raw = cliente.get("FECHA_COMPROMISO_CI")
                                    
                                    # Si la fecha es nula, vacía o NaT, usamos hoy por defecto
                                    if pd.isna(f_comp_raw) or f_comp_raw is None:
                                        f_default = date.today()
                                    else:
                                        try:
                                            if hasattr(f_comp_raw, "date"):
                                                f_default = f_comp_raw.date()
                                            elif isinstance(f_comp_raw, str):
                                                f_default = datetime.strptime(f_comp_raw, "%Y-%m-%d").date()
                                            else:
                                                f_default = f_comp_raw
                                        except:
                                            f_default = date.today()

                                with c_cred2:
                                    st.write(f"🏦 **Entidad:** {cliente.get(c_entidad, 'No registrada')}")

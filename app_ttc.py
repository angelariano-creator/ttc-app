                ...
                if seleccion:
                    ...
                    if cuota_inicial_real > 0:
                        ...
                        if seccion == "📋 Contacto":
                            ...
                            # 1. ACORDEÓN DE CONFIGURACIÓN
                            with st.expander("⚙️ Configurar / Editar Cuotas"):

                                # ---------- SELECTOR CUOTAS ----------
                                col_sel, col_btn_gen, _ = st.columns([0.6,1,3])
                                ...
                                with col_btn_gen:
                                    ...
                                    if st.button("⚡ Generar cuotas automáticamente"):

                                        valor_total = cuota_inicial_automatica
                                        fecha_inicio = date.today()

                                        valor_base = round(valor_total / cuotas_pactadas)

                                        for i in range(1, cuotas_pactadas + 1):

                                            fecha_calc = fecha_inicio + relativedelta(months=i-1)

                                            if i == cuotas_pactadas:
                                                valor_real = valor_total - (valor_base * (cuotas_pactadas - 1))
                                            else:
                                                valor_real = valor_base

                                            st.session_state[f"fecha_c{i}"] = fecha_calc
                                            st.session_state[f"valor_c{i}"] = formato_moneda(valor_real)
                                            st.session_state[f"estado_c{i}"] = "PENDIENTE"
                                ...
                                for i in range(1, cuotas_pactadas + 1):
                                    ...
                                    with c1:
                                        ...
                                        nueva_f = st.date_input(
                                            f"f{i}",
                                            value=st.session_state.get(f"fecha_c{i}", date.today()),
                                            key=f"fecha_c{i}",
                                    ...
                                    with c2:
                                        ...
                                        nuevo_v = st.text_input(
                                            f"v{i}",
                                            value=st.session_state.get(f"valor_c{i}", formato_moneda(0)),
                                            key=f"valor_c{i}",
                                            label_visibility="collapsed"
                                        )
                                    ...
                                    if st.button("💾 Guardar Plan", type="primary", use_container_width=True):
                                        ...
                                        for i in range(1,13):
                                            ...
                                            if i <= cuotas_pactadas:
                                                ...
                                                st.session_state.db.at[idx_sel,f"VALOR_C{i}"] = limpiar_valor(
                                                    st.session_state[f"valor_c{i}"]
                                                )

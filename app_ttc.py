...
if not st.session_state.autenticado:
    ...
    if st.session_state.area_seleccionada == "COMERCIAL":
        ...
        # --- AQUÍ EMPIEZAN TUS FUNCIONES ORIGINALES ---
            ...
            # --- TAB 1: BUSCADOR DINÁMICO (CORREGIDO) ---
            with tabs[0]:
                st.markdown("### 🔎 Central de Información")
                
                # 1. Creamos el diccionario de datos (Apto - Nombre)
                # Usamos el guion bajo correctamente: opciones_full
                opciones_full = {f"Apto {row[c_apto]} - {row[c_nom]}": idx for idx, row in st.session_state.db.iterrows()}
                
                # 2. CUADRO DE TEXTO PARA FILTRAR (Aquí es donde escribes)
                busqueda = st.text_input("🔍 Escribe el Nombre o el Apto para buscar:", placeholder="Ej: 101 o Perez...").upper()
                
                # 3. Filtramos las opciones según lo que escribiste arriba
                opciones_filtradas = [
                    opt for opt in opciones_full.keys() 
                    if busqueda in opt.upper()
                ] if busqueda else list(opciones_full.keys())
                
                # 4. El selector ahora solo muestra lo que coincide con tu búsqueda
                seleccion = st.selectbox(
                    f"🎯 Resultados encontrados ({len(opciones_filtradas)}):",
                    options=["-- Seleccionar --"] + opciones_filtradas,
                    index=0
                )

                # 5. Si seleccionas a alguien de la lista, carga el resto
                    ...
                    if cuota_inicial_real > 0:
                        ...
                        # ===============================
                            ...
                            if pd.notnull(fecha_compromiso_guardada):
                                ...
                                if pd.isna(fecha_base) or fecha_base is None:
                                    dias_diferencia = 0
                                else:
                                    # Si fecha_base es un objeto datetime, lo convertimos a date

...
# Nombre del archivo para prospectos
ARCHIVO_PROSPECTOS = "prospectos_alameda.csv"

# Inicializar la base de prospectos en la memoria
if 'prospectos' not in st.session_state:
    if os.path.exists(ARCHIVO_PROSPECTOS):
        ...
        # Definimos las columnas que necesita un vendedor
        columnas_p = ["FECHA", "NOMBRE", "CELULAR", "CORREO", "PROYECTO_INTERES", "ESTADO", "NOTAS"]
        st.session_state.prospectos = pd.DataFrame(columns=columnas_p)

# Función para guardar en el CSV de prospectos
def guardar_prospectos():
    ...
    if st.session_state.area_seleccionada == "COMERCIAL":
        ...
        # --- AQUÍ EMPIEZAN TUS FUNCIONES ORIGINALES ---
            ...
            with tabs[3]:
                ...
                # 1. MAPEADO DE COLUMNAS RECIÉN VERIFICADO
                ...
                if col_venta and col_recaudo:
                    ...
                    subs_efectivos = df.apply(lambda r: limpiar_valor(r[col_val_sub]) if "APROBADO" in str(r[col_est_sub]).upper() else 0, axis=1).sum()
                    cred_efectivos = df.apply(lambda r: limpiar_valor(r[col_val_cred]) if "SI" in str(r[col_desembolso]).upper() else 0, axis=1).sum()
                    ...
                    # 2. SEMÁFORO DE ESTADO GLOBAL
                    st.divider()
                    c_sem, c_txt = st.columns([1, 4])
                    with c_sem:
                        if porcentaje_recaudo > 0.8:
                            st.markdown("## 🟢") # Verde: Excelente
                        elif porcentaje_recaudo > 0.5:
                            st.markdown("## 🟡") # Amarillo: En proceso
                        else:
                            st.markdown("## 🔴") # Rojo: Alerta de recaudo
                    with c_txt:
                        st.subheader(f"Estado del Recaudo: {porcentaje_recaudo*100:.1f}%")
                        st.write(f"Cartera total por recuperar: **${cartera_pendiente:,.0f}**")

                    # 3. FILTROS DE ALERTA DE GESTIÓN (Apartamentos específicos)
                    st.divider()
                    ...
                    with tab_sub:
                        # Apartamentos con valor de subsidio pero que NO están aprobados
                        mask_sub = (df[col_est_sub].astype(str).str.upper() != "APROBADO") & (df[col_val_sub].apply(limpiar_valor) > 0)
                        df_sub_pend = df[mask_sub][[col_apto, col_nombre, col_est_sub, col_val_sub]].copy()
                        if not df_sub_pend.empty:
                            st.error(f"Se encontraron {len(df_sub_pend)} apartamentos con subsidios sin legalizar:")
                            st.dataframe(df_sub_pend, use_container_width=True, hide_index=True)
                        else:
                            st.success("No hay subsidios pendientes de aprobación.")

                    with tab_cred:
                        # Apartamentos con valor de crédito pero que dicen NO en desembolso
                        mask_cred = (df[col_desembolso].astype(str).str.upper() != "SI") & (df[col_val_cred].apply(limpiar_valor) > 0)
                        df_cred_pend = df[mask_cred][[col_apto, col_nombre, col_val_cred, col_desembolso]].copy()
                        if not df_cred_pend.empty:
                            st.warning(f"Se encontraron {len(df_cred_pend)} apartamentos con créditos pendientes de desembolso:")
                            st.dataframe(df_cred_pend, use_container_width=True, hide_index=True)
                        else:
                            st.success("Todos los créditos han sido desembolsados.")
                    # --- REPORTE HTML ---
                    st.divider()
                    st.subheader("🖨️ Exportar Informe Profesional")

                    # Función interna rápida para el reporte para evitar errores de NaN
                    def fmt_num(val):
                        try:
                    ...
                    # Pre-calculamos los strings de las tablas para que el HTML sea ligero
                    filas_sub = ""
                    for _, r in df_sub_pend.iterrows():
                        filas_sub += f"""
                        <tr class='error-row'>
                            <td>{r[col_apto]}</td>
                            <td>{r[col_nombre]}</td>
                            <td>{r[col_est_sub]}</td>
                            <td>${fmt_num(r[col_val_sub]):,.0f}</td>
                        </tr>"""

                    filas_cred = ""
                    for _, r in df_cred_pend.iterrows():
                        filas_cred += f"""
                        <tr class='warning-row'>
                            <td>{r[col_apto]}</td>
                            <td>{r[col_nombre]}</td>
                            <td>${fmt_num(r[col_val_cred]):,.0f}</td>
                            <td>{r[col_desembolso]}</td>
                        </tr>"""
                    ...
                    <head>
                        ...
                        <style>
                            ...
                            .header {{ text-align: center; border-bottom: 3px solid #2E86C1; margin-bottom: 30px; padding-bottom: 10px; }}
                            .stats {{ display: flex; justify-content: space-around; margin-bottom: 30px; }}
                            .card {{ border: 1px solid #eee; padding: 20px; border-radius: 10px; box-shadow: 2px 2px 10px #eee; width: 30%; text-align: center; }}
                            .val {{ font-size: 22px; font-weight: bold; color: #2E86C1; }}
                            table {{ width: 100%; border-collapse: collapse; margin-top: 20px; }}
                            th, td {{ border: 1px solid #ddd; padding: 12px; text-align: left; }}
                            th {{ background-color: #2E86C1; color: white; }}
                            .error-row {{ background-color: #fff5f5; }}
                            .warning-row {{ background-color: #fffdf0; }}
                            .footer {{ margin-top: 50px; font-size: 12px; text-align: center; color: #888; }}
                        </style>
                    </head>
                    <body>
                        <div class="header">
                            <h1>REPORTE GERENCIAL - PROYECTO ALAMEDA</h1>
                            <p>Generado el: {datetime.now().strftime('%d/%m/%Y %H:%M')}</p>
                        </div>

                        <div class="stats">
                            <div class="card"><div>VENTA TOTAL</div><div class="val">${total_ventas:,.0f}</div></div>
                            <div class="card"><div>RECAUDO REAL</div><div class="val">${recaudo_total:,.0f}</div></div>
                            <div class="card"><div>CARTERA</div><div class="val">${cartera_pendiente:,.0f}</div></div>
                        </div>

                        <h3>⚠️ SUBSIDIOS POR GESTIONAR</h3>
                        <table>
                            <thead><tr><th>Apto</th><th>Nombre</th><th>Estado</th><th>Valor Subsidio</th></tr></thead>
                            <tbody>{filas_sub}</tbody>
                        </table>

                        <h3>⚠️ CRÉDITOS PENDIENTES</h3>
                        <table>
                            <thead><tr><th>Apto</th><th>Nombre</th><th>Valor Crédito</th><th>Desembolso</th></tr></thead>
                            <tbody>{filas_cred}</tbody>
                        </table>

                        <div class="footer">Este documento es un reporte interno de gestión de cartera.</div>
                    </body>
                    </html>
                    """

                    st.download_button(
                        label="📩 DESCARGAR INFORME LIMPIO (PDF/HTML)",
                        data=html_report,
                        file_name=f"Reporte_General_Alameda.html",
                        mime="text/html",
                        use_container_width=True
                    )
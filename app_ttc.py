    ...
    if st.session_state.area_seleccionada == "COMERCIAL":
        ...
        # --- AQUÍ EMPIEZAN TUS FUNCIONES ORIGINALES ---
                    ...
                    if not resultados.empty:
                        ...
                        if seleccion != "-- Seleccionar --":
                            ...
                            # ===============================
                                ...
                                col4.metric(
                                    "💵 Abonado Cuota Inicial",
                                    formato_moneda(total_cancelado_ci_real)
                                )

                                col5.metric(
                                    "📊 Cumplimiento Cuota Inicial",
                                    f"{porcentaje_ci:.2f}%"
                                )
                                ...
                                if fecha_compromiso_guardada:
                                    ...
                                    if hasattr(fecha_compromiso_guardada, "date"):
                                        fecha_compromiso_guardada = fecha_compromiso_guardada.date()
                                    ...
                                    if fecha_compromiso_guardada:
                                        ...
                                        dias_vencido = (date.today() - fecha_compromiso_guardada).days
                                ...
                                if fecha_compromiso_guardada:
                                    if isinstance(fecha_compromiso_guardada, str):
                                        try:
                                            ...
                                            fecha_compromiso_guardada = None
                                # -------- VALIDAR ESTADO --------
                                if pd.notnull(fecha_compromiso_guardada):

                                    fecha_base = pd.to_datetime(fecha_compromiso_guardada).date()
                                    dias_diferencia = (date.today() - fecha_base).days
            ...
            with tabs[1]:
                ...
                # 1. CONTROL DE SEGURIDAD (Para evitar el doble clic)
                if 'procesando_registro' not in st.session_state:
                    st.session_state.procesando_registro = False

                # 2. LISTA DE DISPONIBLES
                df_actual = st.session_state.db
                mask_disponibles = (
                    (df_actual[c_nom].isna()) | 
                    (df_actual[c_nom].astype(str).str.strip() == "") |
                    (df_actual["ESTADO"].astype(str).str.upper() == "DISPONIBLE")
                )
                lista_disponibles = sorted(df_actual[mask_disponibles][c_apto].unique().tolist())

                if not lista_disponibles:
                    st.error("❌ No hay apartamentos disponibles para registro.")
                else:
                    # Usamos st.form para que los datos no "salten" al siguiente apto al hacer clic
                    with st.form("form_registro_completo", clear_on_submit=True):
                        st.markdown("#### 📑 Información Básica del Inmueble")
                        col1, col2, col3 = st.columns(3)
                        with col1:
                            nuevo_apto = st.selectbox("Apartamento:", lista_disponibles)
                        with col2:
                            nuevo_torre = st.selectbox("Torre:", [3])
                        with col3:
                        ...
                        with c2:
                            ...
                            nuevo_estado = st.selectbox("Estado Inicial:", ["RESERVADO", "VENDIDO"])

                        st.markdown("---")
                        st.markdown("#### 💰 Condiciones Comerciales")
                        v1, v2 = st.columns(2)
                        with v1:
                            # Intentamos traer el valor base del Excel para ese apto
                            idx_temp = df_actual[df_actual[c_apto] == nuevo_apto].index[0]
                            valor_base = limpiar_valor(df_actual.at[idx_temp, c_v_venta])
                            nuevo_valor = st.number_input("Valor de Venta ($):", min_value=0, value=int(valor_base), step=1000000)
                        with v2:
                            nuevo_descuento = st.text_input("Descuento Concedido:", value="0")

                        # Botón de envío del formulario
                        submit = st.form_submit_button("🚀 REGISTRAR NUEVO CLIENTE", use_container_width=True)

                    # 3. LÓGICA DE PROCESAMIENTO TRAS EL CLIC
                    if submit:
                        # Validar si ya hay un proceso en marcha
                        if st.session_state.procesando_registro:
                            st.warning("⚠️ Ya se está procesando un registro. Por favor espere.")
                        else:
                            if nuevo_apto and nuevo_nombre and nueva_cedula:
                                # ACTIVAR CANDADO
                                st.session_state.procesando_registro = True
                                
                                try:
                                    # VERIFICACIÓN FINAL: ¿El apto sigue vacío en la DB real?
                                    idx = st.session_state.db[st.session_state.db[c_apto] == nuevo_apto].index[0]
                                    check_propietario = str(st.session_state.db.at[idx, c_nom]).strip()

                                    if check_propietario == "" or check_propietario.lower() == "nan":
                                        # ACTUALIZAR DATOS EN EL DATAFRAME
                                        st.session_state.db.at[idx, c_nom] = nuevo_nombre.upper()
                                        st.session_state.db.at[idx, "CEDULA"] = nueva_cedula
                                        st.session_state.db.at[idx, "CELULAR"] = nuevo_celular
                                        st.session_state.db.at[idx, "E-MAIL"] = nuevo_email
                                        st.session_state.db.at[idx, "CIUDAD"] = nueva_ciudad
                                        st.session_state.db.at[idx, "ESTADO"] = nuevo_estado
                                        st.session_state.db.at[idx, c_v_venta] = nuevo_valor
                                        st.session_state.db.at[idx, "DESCUENTO"] = nuevo_descuento
                                        st.session_state.db.at[idx, "ASESOR"] = nuevo_asesor.upper()

                                        # VENTANA EMERGENTE (Modal de éxito)
                                        @st.dialog("✅ Registro Exitoso")
                                        def mensaje_exito(nombre, apto):
                                            st.write(f"El cliente **{nombre}** ha sido vinculado correctamente al apartamento **{apto}**.")
                                            if st.button("Aceptar"):
                                                st.session_state.procesando_registro = False
                                                st.rerun()

                                        mensaje_exito(nuevo_nombre.upper(), nuevo_apto)

                                    else:
                                        st.error(f"❌ El apto {nuevo_apto} ya fue registrado por otro usuario.")
                                        st.session_state.procesando_registro = False
            ...
            with tabs[2]:
                ...
                for idx, row in st.session_state.db.iterrows():
                    ...
                    # --- Lógica de Fechas y Mora ---
                    fecha_compromiso = row.get(c_fecha_pago) 
                    dias_mora = 0
                    if pd.notnull(fecha_compromiso) and fecha_compromiso != "":
                        try:
                            if hasattr(fecha_compromiso, 'date'): f_base = fecha_compromiso.date()
                            elif isinstance(fecha_compromiso, str): f_base = pd.to_datetime(fecha_compromiso).date()
                            else: f_base = fecha_compromiso
                            diferencia = (date.today() - f_base).days
                            dias_mora = diferencia if diferencia > 0 else 0
                        except: dias_mora = 0

...
def login():
    # Usamos columnas con ratios más adaptables: [0.1, 0.8, 0.1] para que en móvil use casi todo el ancho
    _, col_central, _ = st.columns([0.1, 0.8, 0.1])
    
    with col_central:
        # Contenedor para agrupar visualmente el acceso
        with st.container(border=True):
            # Centramos la imagen y el título con un poco de HTML para que se vea bien en móvil
            ...
            with st.form("login_form", clear_on_submit=False):
                ...
                # El botón con use_container_width es clave para celulares
                boton_login = st.form_submit_button("INGRESAR AL SISTEMA", use_container_width=True)
                
                if boton_login:
                    # Normalizamos entradas para evitar errores de mayúsculas/minúsculas
                    u_clean = u.strip()
    ...
    if st.session_state.area_seleccionada == "COMERCIAL":
        ...
        # --- AQUÍ EMPIEZAN TUS FUNCIONES ORIGINALES ---
            ...
            with tabs[0]:
                ...
                # 1. Definimos el ancho (25% para el buscador, 75% espacio libre)
                col_busq, col_espacio = st.columns([1.5, 4.5]) 
                
                with col_busq:
                    # Preparamos las opciones (Apto - Nombre)
                    opciones_full = {f"Apto {row[c_apto]} - {row[c_nom]}": idx for idx, row in st.session_state.db.iterrows()}
                    
                    # BUSCADOR INTELIGENTE:
                    # index=None hace que empiece vacío (sin "-- Seleccionar --")
                    # placeholder es el texto gris transparente que invita a escribir

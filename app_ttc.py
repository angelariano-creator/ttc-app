    ...
    if st.session_state.area_seleccionada == "COMERCIAL":
        ...
        # --- AQUÍ EMPIEZAN TUS FUNCIONES ORIGINALES ---
            ...
            with tabs[2]:
                ...
                if data_cartera and isinstance(data_cartera, list) and len(data_cartera) > 0:

                    st.write("### 📋 Gestión de Cobro Prioritaria")

                    df_export = pd.DataFrame(data_cartera)

                    if not df_export.empty:

                        df_export["Saldo Pendiente"] = pd.to_numeric(
                            df_export["Saldo Pendiente"], errors="coerce"
                        ).fillna(0)

                        df_export = df_export.sort_values(
                            by="Saldo Pendiente", ascending=False
                        )

                        for _, fila in df_export.iterrows():

                            saldo = float(fila.get("Saldo Pendiente", 0))
                            color_alerta = "#e74c3c" if saldo > 50000000 else "#f39c12"
                            ...
                            with st.container(border=True):
                                ...
                                with c_inf:
                                    st.markdown(f"**Unidad: {fila.get('Inmueble', '')}**")
                                    st.markdown(f"👤 {fila.get('Cliente', '')}")
                                    st.caption(
                                        f"Asesor: {fila.get('Asesor', '')} | Estado: {fila.get('Estado', '')}"
                                    )
                                ...
                                with c_monto:
                                    st.markdown(
                                        ...
                                        <div style='text-align:right;'>
                                            <span style='color:{color_alerta}; font-size:1.2rem; font-weight:bold;'>
                                                {formato_moneda(saldo)}
                                            </span><br>
                                            <small style='color:gray;'>SALDO PENDIENTE</small>
                                        </div>
                                        """,
                                        unsafe_allow_html=True
                                    )
                ...
                if not col_venta or not col_recaudo:
                    st.error("No se encontraron las columnas obligatorias: VALOR VENTA o TOTAL CANCELADO C.I")
                    st.stop()

                # Normalización preventiva de fecha (evita errores en comparaciones)
                if col_fecha_ci:
                    df[col_fecha_ci] = pd.to_datetime(df[col_fecha_ci], errors="coerce")
                    ...
                    for _, fila in df.iterrows():
                        ...
                        # Validar que la fecha exista y no sea NaT
                        if pd.notna(fecha_ci):

                            # Convertir siempre con pandas (funciona para str, Timestamp, etc.)
                            fecha_ci = pd.to_datetime(fecha_ci, errors="coerce")

                            if pd.notna(fecha_ci):

                                fecha_ci = fecha_ci.date()

                                # Comparación segura
                                if fecha_ci < date.today() and float(abonado) < float(cuota_requerida):
                                    cartera_vencida += 1

                                if fecha_ci == mañana and float(abonado) < float(cuota_requerida):
                                    vencen_manana += 1
                    ...
                    # ===============================
                    # 9️⃣ INFORME EJECUTIVO PROFESIONAL
                    # ===============================

                    st.divider()
                    st.subheader("📊 Informe Ejecutivo Gerencial")

                    # ===============================
                    # RESUMEN TORRE 3 DESDE CONTROLLO VENDITE
                    # ===============================

                    df_torre3_control = pd.read_excel(
                        "CONTROL CLIENTES ALAMEDA.xlsx",
                        sheet_name="CONTROLLO VENDITE TORRE 3"
                    )

                    control = df_torre3_control.dropna(subset=["DISPONIBILE"])
                    ultima_fila = control.iloc[-1]

                    disp = int(ultima_fila["DISPONIBILE"])
                    reservado = int(ultima_fila["RESERVADO"])
                    separado = int(ultima_fila["SEPARADO"])
                    tramite = int(ultima_fila["TRAMITE"])
                    aprobado = int(ultima_fila["APPROBADO"])
                    desembolso = int(ultima_fila["DESEMBOLSO"])
                    entrega = int(ultima_fila["ENTREGA"])
                    total_unidades = int(ultima_fila["TOTALE"])

                    vendidas = total_unidades - disp

                    if total_unidades > 0:
                        porcentaje_ocupacion = round((vendidas / total_unidades) * 100, 1)
                    else:
                    ...
                    <div class="header">
                        <h1>INFORME EJECUTIVO GERENCIAL</h1>
                    ...
                    <div class="kpi">
                        <h2>{formato_moneda(total_ventas)}</h2>
                        <p>Venta Total</p>
                    </div>

                    <div class="kpi">
                        <h2>{formato_moneda(recaudo_total)}</h2>
                        <p>Recaudo Total</p>
                    </div>

                    <div class="kpi">
                        <h2>{formato_moneda(cartera_pendiente)}</h2>
                        <p>Cartera Pendiente</p>
                    </div>

                    <div class="kpi">
                        <h2>{cartera_vencida}</h2>
                        <p>Cuotas Iniciales Vencidas</p>
                    </div>
                    </div>

                    <h2 class="section-title">🏢 Estado Comercial Torre 3</h2>

                    <div class="kpi-container">
                    <div class="kpi ocupacion">
                        <h2>{porcentaje_ocupacion}%</h2>
                        <p>Ocupación del Proyecto</p>
                    </div>
                    ...
                    <td>{vendidas}</td>
                    <td>{disp}</td>
                    <td>{porcentaje_ocupacion}%</td>
                    </tr>
                    </table>

                    <div class="alerta">
                    <strong>⚠ Alertas Importantes:</strong><br><br>
                    • Cuotas que vencen mañana: <strong>{vencen_manana}</strong><br>
                    • Cuotas vencidas: <strong>{cartera_vencida}</strong>
                    </div>
                    ...
                    # PREVISUALIZACIÓN
                    if st.button("👁️ Previsualizar Informe"):
                    ...
                    st.download_button(
                        ...
                        data=html_report,
                        file_name="Informe_Ejecutivo_Torre3.html",
                        mime="text/html",
                        use_container_width=True
                    )